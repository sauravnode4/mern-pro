...
const handleUserSignup=async(req,res)=>{
    ...
    try {
        ...
        if(isUser){
             return res.status(400).json({status:false , message:"with this email user alredy exist"});
        }
...
const handleUserLogin=async(req,res)=>{
     if(req.body == undefined){
        return res.status(400).json({status:false , message:"without details user cannot be login"});
    }
     try {
        ...
        const token=jwt.sign(payload , 'MERN' ,{expiresIn:"1d"});
        return res.status(200).json({status:true,message:"Login SUccessfull",token});
    } catch (error) {
        return res.status(500).json({status:false , message:"server side error"});
    }
}
...
module.exports={handleUserSignup,handleUserLogin,getUserInfo ,handleUserUpdateName,handleUserUpdatePassword}